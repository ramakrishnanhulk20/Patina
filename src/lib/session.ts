 ...
 * it and start again, and clearing it costs nothing: the durable identity is the
 * Personal Server behind the Vana approval (see profileForServer in store.ts),
 * which reunites a person with their profile on any machine they connect from.
 * This cookie only saves them the round trip.
 *
 ...
 *
 * The prefix also did the migration away from that. Every cookie issued before
 * it holds a bare 32-character profile id, fails this test, and stopped being
 * honoured the moment the change deployed. Those values had been published, and
 * there was no way to tell a person holding their own from a person holding one
 * they scraped.
 *
 * Recovery no longer needs a sign-in. Connecting a source proves which Personal
 * Server the browser belongs to, and the profile comes back with it.
 */
